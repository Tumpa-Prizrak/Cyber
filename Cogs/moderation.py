...
class ModerationCommand(commands.Cog):
    ...
    async def ban(self, ctx, man: discord.Member, *, reason = "Bad guy"):
        ...
        emb = discord.Embed(title=f"**{man.display_name}** ({man}) был забанен", colour=discord.colour.Color.green())
        await ctx.send(embed=emb)
    ...
    async def kick(self, ctx, man: discord.Member, *, reason: str = "Bad guy"):
        ...
        emb = discord.Embed(title=f"**{man.display_name}** ({man}) был кикнут", colour=discord.colour.Color.green())
        await ctx.send(embed=emb)
