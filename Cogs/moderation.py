...
class ModerationCommand(commands.Cog):
    ...
    async def cog_command_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            emb = nextcord.Embed(title="Бро, у тебя нет прав", colour=nextcord.colour.Colour.red())
            await ctx.send(embed=emb)
        elif isinstance(error, commands.MissingRequiredArgument):
            try:
                emb = nextcord.Embed(title=f"Параметр \"{self.someShit[error.param.name]}\" пропущен", colour=nextcord.colour.Colour.red())
                await ctx.send(embed=emb)
            except KeyError:
                emb = nextcord.Embed(title=f"Параметр \"{error.param.name}\" пропущен", colour=nextcord.colour.Colour.red())
                await ctx.send(embed=emb)
        elif isinstance(error, nextcord.Forbidden):
            emb = nextcord.Embed(title="Я не могу это сделать, прав не хватает :sob:")
            await ctx.send(embed=emb)
        else:
            raise error
    ...
    async def ban(self, ctx, man: nextcord.Member, *, reason = "Bad guy"):
        if man == ctx.author:
            emb = nextcord.Embed(title="Вы не можете сделать это с собой  :sob:", colour=nextcord.colour.Colour.red())
        ...
        emb = nextcord.Embed(title=f"**{man.display_name}** был забанен", colour=nextcord.colour.Color.green())
        await ctx.send(embed=emb)
    ...
    async def kick(self, ctx, man: nextcord.Member, *, reason: str = "Bad guy"):
        if man == ctx.author:
            emb = nextcord.Embed(title="Вы не можете сделать это с собой  :sob:", colour=nextcord.colour.Colour.red())
            return await ctx.send(embed=emb)
        if ctx.author.top_role.position <= man.top_role.position or ctx.guild.owner != ctx.author:
        ...
        emb = nextcord.Embed(title=f"**{man.display_name}** был кикнут", colour=nextcord.colour.Color.green())
        await ctx.send(embed=emb)

    # mute временно на доработке, сорян
    # @commands.command()
    # @commands.has_permissions(manage_roles=True)
    # async def mute(self, ctx, man: nextcord.Member):
    ...
    #     conn.commit()
    #     embed = nextcord.Embed(title=f"\"{man.mention}\" был замьючен",
    #                           colour=nextcord.colour.Color.green())
    ...
    # @commands.has_permissions(manage_roles=True)
    # async def warn(self, ctx, man: nextcord.Member, *, reason: str):
    #    if ctx.author.top_role.position <= man.top_role.position and ctx.guild.owner != ctx.author:
    #        raise commands.MissingPermissions
    #    await self.db.insert_one({'guild': ctx.guild.id, 'member': man.id}, {'$push': {'warns': reason}}) # we need id's
    #    embed = nextcord.Embed(title=f"**{man.mention}** ({man})  получил предупреждение", colour=nextcord.colour.Color.green())
    #    await ctx.send(embed=embed)
    ...
    # async def warns(self, ctx, man: nextcord.Member = None):
    ...
    #             all_warns += "`" + i[2] + "`"
    #             all_warns += ";"
    #     embed = nextcord.Embed(title=f'Предупреждения {man.display_name if man else ctx.author.display_name}: {all_warns}', colour=nextcord.colour.Color.green())
    #     await ctx.send(embed=embed)
