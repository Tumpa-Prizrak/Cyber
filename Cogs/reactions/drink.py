...
class ReactionsCommand(commands.Cog):
    ...
    @commands.command()
    async def drink(self, ctx, person: discord.Member = None):
        r = requests.get(f"https://g.tenor.com/v1/search?q=anime drink tea&key={c.apikey}&limit={str(c.limit)}")

        if r:
            emb = discord.Embed(
                title=f"{ctx.author.name} пьёт" if person == None else f"{ctx.author.name} пьёт с {person.display_name}",
